...
#   SYNC_INTERVAL  – seconds between sync loops (default: 300)
#
# Usage:
#   docker run --rm \
...
# Validate required variables early so failures are obvious.
if [ -z "${REPO:-}" ]; then
	echo "ERROR: REPO environment variable is required (e.g. git@host:org/repo.git)" >&2
	exit 1
fi
...
cd "$GITDIR"
git config user.email "${GIT_USER_EMAIL:-opencode@daveshow.local}"
git config user.name "${GIT_USER_NAME:-opencode-sync}"
git config core.sshcommand "ssh -i ${SSH_KEY_PATH} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"
